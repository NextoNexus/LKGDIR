...
def test(dir_path, device, atlas_dir, model_load, current_epoch, reduc_size, slurm_data_path):
    ...
    val_datasets = Unaligned_Datasets.IxIDataset_for_atlas2img(slurm_data_path=slurm_data_path, cases_dir='testset',
                                                               need_seg=True, reduce_size=reduc_size)
    val_dataloader = DataLoader(val_datasets, batch_size=1, shuffle=False)

    good_labels = [2, 3, 4, 7, 8, 10, 11, 12, 13, 14, 15, 16, 17, 18, 24, 28, 41, 42, 43, 46, 47, 49, 50, 51, 52, 53,
                   54, 60]
    ...
    with torch.no_grad():
        for k in range(0, val_datasets.__len__()):
            ...
            warpped_seg_arr = transform(atlas_seg_tensor, flow.permute(0, 2, 3, 4, 1),
                                        mod='nearest').detach().cpu().numpy()
            ...
            vals_all = [(dices[0] + dices[16]) / 2, (dices[1] + dices[17]) / 2, (dices[2] + dices[18]) / 2,
                        ...
                        (dices[12] + dices[25]) / 2, (dices[13] + dices[26]) / 2, dices[14],
                        (dices[15] + dices[27]) / 2]

            vals_all = [round(i, 9) for i in vals_all]
            print('eval done. the Dice for {}th image is :{}----------'.format(k + 1, np.mean(
                vals_all)))
    ...
    return mean_dice_for_brainRegion, mean_dice_for_subject, inference_time,num_params


if __name__ == "__main__":
    parser = ArgumentParser()
    parser.add_argument("--dir_path", type=str, default='./Trials/Trial_4', help="trial dir")
    ...
    parser.add_argument("--model_load", type=str, dest="model_load", help="Model path to be loaded",
                        default=r'checkpoint/TrainLoss_-0.317451_Epoch_100_checkpoint')
    parser.add_argument("--current_epoch", type=int, dest="current_epoch", help="current epoch of trained model",
                        default=None)
    ...
    mean_dice_for_brainRegion, mean_dice_for_subject, inference_time,num_params = test(**vars(parser.parse_args()))
    print(mean_dice_for_brainRegion)
    print(mean_dice_for_subject)
    print('Evaluation Result-------------------------->')
    print('mean DSC:',np.mean(mean_dice_for_subject))
    print('std:',np.std(mean_dice_for_subject))
